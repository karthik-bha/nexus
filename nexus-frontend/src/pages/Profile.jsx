...
/// What to display
// Settings
// pfp in center with a edit option
// username with edit icon
// email if available else just a gentle nudge to input if needed (provide add and edit option for non-existent ones)
// reset password
// bio with add/edit/delete option
...
const Profile = () => {
    const mockData = {
        ...
        created_at: "2025-09-24T09:59:23.094+00:00",
    }
    ...
    return (
        <section className="max-w-[1200px] mx-auto flex flex-col py-12 px-4">
            ...
            <div className="flex flex-col items-center py-6 gap-4 md:w-[40vw]  md:mx-auto">
                <img src={mockData.profile_picture}
                    alt="profile_picture"
                    className="h-32 w-32"
                />
                <div className="flex flex-col gap-4">
                    <p>{mockData?.username || "Username not available"}</p>
                    {mockData?.email ? <>
                        <p>{mockData.email}</p>
                    </> : <>
                        <p className="text-red-500 text-[0.9rem] italic">No e-mail found</p>
                    </>}
                    <p className={mockData?.bio ? "" : `text-red-500 text-[0.9rem] italic`}>{mockData?.bio || "No bio found"}</p>
                    <div className="flex gap-2 flex-col md:flex-row">
                        <button className="button_style_2 text-[0.9rem]">Reset password</button>
                        <button className="button_style_2 text-[0.9rem]">Edit profile</button>
                    </div>
